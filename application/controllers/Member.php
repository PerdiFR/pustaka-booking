        ...
        if ($user) {
            if ($user['is_active'] == 1) {
                if (password_verify($password, $user['password'])) {
                    ...
                    $this->_setFlashMessage('danger', 'Password salah!!');
                    redirect('home');
                }
            } else {
                $this->_setFlashMessage('danger', 'User  belum diaktifasi!!');
                redirect('home');
            }
        } else {
            $this->_setFlashMessage('danger', 'Email tidak terdaftar!!');
            redirect('home');
        }
    }

    private function _setFlashMessage($type, $message)
    {
        $this->session->set_flashdata('pesan', "<div class='alert alert-$type alert-message' role='alert'>$message</div>");
    }
